...
<html>
    <head>
        ...
        <title>Olympic Picture</title>
    </head>
    <body>
        <h1><center>Olympic Picture Search</center></h1>
        <%
            if ((Boolean) request.getAttribute("isConnected")) {
                if (request.getAttribute("pictureURL").equals("no pictures")) {
        %><p>Sorry, no pictures in this sport.</p>
        <%} else {%>
        <p>Here is picture of an athlete playing <%= request.getAttribute("sportName")%></p><br>
        <img <%= request.getAttribute("pictureURL")%>><br><br>
        <% }
        } else {%>
        <p>Sorry, fail to connect to Olympic.org.Try Again!</p>
        <%}%>
        <form action="OlympicPictureSearchServlet" method="GET">
            <label for="letter">Search another athlete</label>
